...
export class RpcClient implements Host {
  ...
  private handle(data: unknown): void {
    ...
    if ('re' in data) {
      ...
      if (data.ok) {
        ...
        p.reject(new Arm64JSError(data.error.code, data.error.message));
      }
    }
  }
  ...
  private onEvent(ev: RpcEvent): void {
    if (ev.event === 'progress' && ev.re !== undefined) {
      ...
      for (const cb of this.outputs.get(ev.vmId) ?? []) {
        cb(bytes, info);
      }
    } else if (ev.event === 'exit' && ev.vmId) {
      for (const cb of this.exits.get(ev.vmId) ?? []) {
        cb(ev.data as VmExit);
      }
    }
  }

  private async call<T>(op: string, args: unknown[], onProgress?: (p: unknown) => void): Promise<T> {
    if (this.lost) {
      throw new Arm64JSError('vm-lost', 'the VM frame stopped answering');
    }
    const id = ++this.seq;
  ...
  async boot(target: string, opts: BootOptions = {}): Promise<{ vmId: string }> {
    const { onProgress, ...rest } = opts;
    return this.call('boot', [target, rest], onProgress as ((p: unknown) => void) | undefined);
  }

  async exec(vmId: string, command: string, opts: ExecOptions = {}) {
  ...
  async write(vmId: string, data: string): Promise<void> {
    return this.call<void>('write', [vmId, data]);
  }

  async resize(vmId: string, cols: number, rows: number): Promise<void> {
    return this.call<void>('resize', [vmId, cols, rows]);
  }

  async mount(vmId: string, files: MountFiles, path: string): Promise<void> {
    return this.call<void>('mount', [vmId, files, path]);
  }

  async unmount(vmId: string, path: string): Promise<void> {
    return this.call<void>('unmount', [vmId, path]);
  }

  async writeFile(vmId: string, path: string, data: Blob, opts: WriteFileOptions = {}): Promise<void> {
    return this.call<void>('writeFile', [vmId, path, data, opts]);
  }

  async readFile(vmId: string, path: string, opts: ReadFileOptions = {}): Promise<Blob> {
    return this.call<Blob>('readFile', [vmId, path, opts]);
  }
  ...
  async snapshot(vmId: string, opts: SnapshotOptions = {}) {
    const { onProgress, ...rest } = opts;
  ...
  async removeSnapshot(id: string, opts = {}) {
