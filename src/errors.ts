...
// `withSdkErrors`, so a caller sees one `Arm64JSError` class whichever way the VM runs.
...
export async function withSdkErrors<T>(p: Promise<T>): Promise<T> {
...
function rethrow(e: unknown): never {
  ...
  if (e instanceof Arm64JSError) {
    throw e;
  }
  if (code) {
    throw new Arm64JSError(code, (e as Error).message, (e as { data?: unknown }).data);
  }
